...
function normalizeErrorMessage(err: any): { userMessage: string; status: number; errorType: AiCheckErrorType } {
  if (err?.name === 'AiConfigurationError') {
    return {
      userMessage: 'NO CAP AI is not configured. NVIDIA_API_KEY environment variable is missing.',
      status: 503,
  ...
  if (err?.name === 'AiServiceError') {
    return {
      userMessage: err.message || 'NO CAP AI provider encountered an error.',
      status: err.statusCode || 503,
      errorType: err.errorType || 'UNAVAILABLE',
    }
  }

  const msg = (err?.message || String(err)).toLowerCase()

  if (msg.includes('not configured') || msg.includes('missing') || (msg.includes('api key') && msg.includes('nvidia'))) {
    return {
      userMessage: 'NO CAP AI is not configured. NVIDIA_API_KEY environment variable is missing.',
      status: 503,
      errorType: 'NOT_CONFIGURED',
  ...
  if (msg.includes('401') || msg.includes('403') || msg.includes('unauthorized') || msg.includes('authentication')) {
    return {
      userMessage: 'NO CAP AI authentication failed. Please verify the NVIDIA API key.',
      status: 401,
  ...
  if (msg.includes('429') || msg.includes('rate limit') || msg.includes('busy') || msg.includes('quota')) {
    return {
      userMessage: 'NO CAP AI is temporarily rate-limited. Please try again in a moment.',
      status: 429,
...
export default async function handler(req: IncomingMessage, res: ServerResponse) {
  ...
  try {
    ...
    if (isSse) {
      ...
      res.setHeader('Content-Type', 'application/json')
      const result = await runNvidiaGemmaCheck(payload)
      res.statusCode = 200
